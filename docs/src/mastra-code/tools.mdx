...
This tool is available through four providers:

- **Parallel**: When the `PARALLEL_API_KEY` environment variable is set.
- **Tavily**: When `TAVILY_API_KEY` is set and no Parallel key is configured.
- **Anthropic web search**: Built into Anthropic models, used when no external provider key is configured.
- **OpenAI web search**: Built into OpenAI models, used when no external provider key is configured.

Set `PARALLEL_API_KEY` to use Parallel as the default external provider. When both external provider keys are set, Mastra Code uses Parallel. The input schema depends on the active provider.

#### Parallel

| Parameter | Type | Required | Description |
| - | - | - | - |
| `searchQueries` | `string[]` | Yes | Concise keyword search queries. Provide 2 to 3 queries for best results. |
...
Extract content from one or more web pages. Requires `PARALLEL_API_KEY` or `TAVILY_API_KEY`. Parallel takes priority when both keys are set.

#### Parallel

| Parameter | Type | Required | Description |
| - | - | - | - |
| `urls` | `string[]` | Yes | URLs to extract content from, up to 20 per request |
| `objective` | `string` | No | Natural-language description of the information to extract |
| `fullContent` | `boolean \| number` | No | Return full page content, optionally limited to a maximum character count |
