...
| Parameter | Type | Required | Description |
| - | - | - | - |
| `search_queries` | `string[]` | Yes | Concise keyword search queries. Provide 2 to 3 queries for best results. |
...
Extract the full content of one or more web pages. Requires `PARALLEL_API_KEY` or `TAVILY_API_KEY`. Parallel takes priority when both keys are set.
